...
static void usage(const char *progm, const char *error)
{
	if (error)
...
{
	EXT_ERRORF("%s: %s"EXT_NEW_LINE, error, strerror(errno));
	exit(1);
}
...
static void sendpacket(int sock, struct sockaddr *addr, socklen_t addr_len)
{
	...
	if (res < 0)
	{
		EXT_ERRORF("send: %s", strerror(errno));
	}
	else
	{
		EXT_INFOF("%ld.%06ld: sent %d bytes\n", (long)now.tv_sec, (long)now.tv_usec, res);
	}
}
			...
			int siocgstamp, int siocgstampns)
{
	...
	for (cmsg = CMSG_FIRSTHDR(msg); cmsg; cmsg = CMSG_NXTHDR(msg, cmsg))
	{
		...
		{
			/* get timestamp, timestampns or timestamping from control message */
			case SOL_SOCKET:/* control message in socket level */
				printf("SOL_SOCKET ");
			...
			case IPPROTO_IP:
				...
				switch (cmsg->cmsg_type)
				{
					case IP_RECVERR:
					{
						...
						else if (!memcmp(sync, data + res - sizeof(sync), sizeof(sync)))
			...
			default:
				printf("Unknonw level %d type %d", cmsg->cmsg_level, cmsg->cmsg_type);
				break;
			}
		printf("\n");
	}

	/* get timestamp or timestampns from socket */
	if (siocgstamp)
	{
...
int main(int argc, char **argv)
{
	...
	/* 2: set hw stamp, configured by socket, not control message */
	...
	{/* mulitple configuration: hw timestamping, from view of driver */
	...
	if (so_timestamp && setsockopt(sock, SOL_SOCKET, SO_TIMESTAMP, &enabled, sizeof(enabled)) < 0)
		bail("setsockopt SO_TIMESTAMP");

	if (so_timestampns && setsockopt(sock, SOL_SOCKET, SO_TIMESTAMPNS, &enabled, sizeof(enabled)) < 0)
		bail("setsockopt SO_TIMESTAMPNS");

	if (so_timestamping_flags && setsockopt(sock, SOL_SOCKET, SO_TIMESTAMPING, &so_timestamping_flags, sizeof(so_timestamping_flags)) < 0)
