...
static int forwarding(struct PtpClock *c, struct PtpPort *p)
{
	enum PORT_STATE ps = portState(p);
	switch (ps) {
	case PS_MASTER:
...
static int clock_do_forward_mgmt(struct PtpClock *c, struct PtpPort *in, struct PtpPort *out,
...
static void clock_forward_mgmt_msg(struct PtpClock *c, struct PtpPort *p, struct ptp_message *msg)
{
