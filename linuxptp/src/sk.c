...
int sk_receive(int fd, void *buf, int buflen, struct address *addr, struct hw_timestamp *hwts, int flags)
{
	...
	struct cmsghdr *cm;	/* control messages, eg. ancillary data from msg */
	...
	if (addr)
	{/* address of this receiving operation */
		msg.msg_name = &addr->ss;
		msg.msg_namelen = sizeof(addr->ss);
	}
	
	msg.msg_iov = &iov;	/* buffer of receiving data */
	msg.msg_iovlen = 1;
	msg.msg_control = control;	/* buffer for control message, ancillary data */
	...
	for (cm = CMSG_FIRSTHDR(&msg); cm != NULL; cm = CMSG_NXTHDR(&msg, cm))
	{
		...
		{/* timestamp for each incoming packet ub system time as struct timespec (nsec) */
