...
static void _nullfDestroy(struct servo *servo)
{
	struct nullf_servo *s = container_of(servo, struct nullf_servo, servo);
	free(s);
}

static double _nullfSample(struct servo *servo, int64_t offset,
...
static void _nullfSyncInterval(struct servo *servo, double interval)
{
}

static void _nullfReset(struct servo *servo)
{
}
...
struct servo *nullf_servo_create(void)
{
	...
	s->servo.destroy = _nullfDestroy;
	s->servo.sample = _nullfSample;
	s->servo.sync_interval = _nullfSyncInterval;
	s->servo.reset = _nullfReset;
